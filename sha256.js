...
function getHash(msgblock) {
  ...
  document.getElementById("hashcode").value = hexToHash(H);
...
function rightShift(n, d) {
  ...
  return (n.substring(0, n.length - d)).padStart(32, "0");
}
function rightRotate(s, r) {

  return s.substring(s.length - r, s.length) + s.substring(0, s.length - r);
}
//XOR 
function xor(a, b, c) {
  let s = "";
  for (let i = 0; i < 32; i++) {
    let count = Number(a.charAt(i)) + Number(b.charAt(i)) + Number(c.charAt(i));
    if (count === 3 || count === 1) {
      s += "1";
    } else {
      s += "0";
...
function generateHash() {
  ...
  getHash(getMsgBlocks(strToBinary(myString)));
}
